...
public class CustomerDAO {
    ...
    public void addFunction(CustomerDTO customerDTO) {
        try {
            ...
            JOptionPane.showMessageDialog(null, "un nouvelle client est ajouter .");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void editCustomerDAO(CustomerDTO customerDTO) {
        try {
            ...
            prepStatement.executeUpdate();
            JOptionPane.showMessageDialog(null, "l'information du client et modifier en succses.");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
    ...
    public void deleteCustomerDAO(String custCode) {
        try {
            ...
            JOptionPane.showMessageDialog(null, "le client est suprimer.");
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
    ...
    public ResultSet getQueryResult(int Limit, int Offset) {
        try {
            String query = "SELECT customercode,fullname,location,phone FROM customers  LIMIT " + Limit + " OFFSET " + Offset;
            resultSet = statement.executeQuery(query);
        } catch (SQLException e) {
            e.printStackTrace();
        }
...
// Method to retrieve search data
    ...
    public ResultSet getCustomerSearch2(String text) {
        try {
            String query = "SELECT customercode,fullname,location,phone FROM customers "
                    + "WHERE customercode LIKE '%" + text + "%' OR fullname LIKE '%" + text + "%' OR "
                    + "location LIKE '%" + text + "%' OR phone LIKE '%" + text + "%'";
    ...
    public int getCustomerCountSearch2(String text) {
