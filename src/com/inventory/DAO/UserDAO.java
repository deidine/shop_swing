...
// Search method for products
    ...
    public boolean canLoan(String username) {
        int canLoan = 0;
        try {
            String query = "SELECT canLoan FROM users where username ='" + username + "'";
            resultSet = statement.executeQuery(query);
            while (resultSet.next()) {
                canLoan = resultSet.getInt("canLoan");
            }
...
//            }
            ...
            String query = "INSERT INTO users (name,location,phone,username,password,usertype,canLoan) "
                    + "VALUES(?,?,?,?,?,?,'"+canLoan+"')";
    ...
    public void editUserDAO(UserDTO userDTO) {
        ...
        try {
            String query = "UPDATE users SET name=?,location=?,phone=?,usertype=?,password=? WHERE username=?";
            prepStatement = conn.prepareStatement(query);
            prepStatement.setString(1, userDTO.getFullName());
            prepStatement.setString(2, userDTO.getLocation());
            prepStatement.setString(3, userDTO.getPhone());
